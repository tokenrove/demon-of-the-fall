...
(defun load-room (name sprite-manager &key (spawn-actors-p t))
  (let ((room (make-instance 'room)))
    ...
    (setf (exits-of room) (cdr (assoc :exits (cdr (equinox::room-archetype room)))))
    (setf (player-spawn-of room)
	  (cadr (assoc :player-spawn (cdr (equinox::room-archetype room)))))
    room))
...
(defun draw-exit (exit ex great)
  (multiple-value-bind (x1 y1)
      (equinox:iso-project-point #I((* equinox::+tile-size+ (+ (caar exit)
	...
	(equinox:iso-project-point #I((* equinox::+tile-size+ (+ (caar exit)
	  ...
	  (equinox:iso-project-point
	   #I((+ (* equinox::+tile-size+ (caar exit))
		 (cond ((and ex (not great)) (- (equinox:half equinox::+tile-size+)))
		       ((not ex) (equinox:half equinox::+tile-size+))
		       (t (+ equinox::+tile-size+ (equinox:half equinox::+tile-size+)))))
	      0
	      (+ (* equinox::+tile-size+ (cdar exit))
		 (cond (ex (equinox:half equinox::+tile-size+))
		       ((not great) (- (equinox:half equinox::+tile-size+)))
		       (t (+ equinox::+tile-size+ (equinox:half equinox::+tile-size+)))))))
